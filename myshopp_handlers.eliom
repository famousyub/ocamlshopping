...
let upload_user_avatar_handler myid () ((), (cropping, photo)) =
  let avatar_dir =
    List.fold_left Filename.concat
      (List.hd !Myshopp_config.avatar_dir)
      (List.tl !Myshopp_config.avatar_dir)
  in
  let%lwt avatar =
...
let%server forgot_password_handler =
  Os_handlers.forgot_password_handler
    Myshopp_services.settings_service
  ...
  | e ->
      ...
      then
        ...
        then
          ...
          in
          Myshopp_base.App.send
            (Myshopp_page.make_page (Os_page.content page))
        else
          let page =
            [ div
                ~a:[a_class ["login-signup-box"]]
...
let%shared settings_handler myid_o () () =
  let%lwt content =
    match myid_o with
    | Some _ -> Myshopp_settings.settings_content ()
    | None -> Lwt.return [p [%i18n log_in_to_see_page ~capitalize:true]]
  in
  Myshopp_container.page myid_o content

let%server update_language_handler () language =
  Os_session.connected_wrapper Myshopp_language.update_language
    (Myshopp_i18n.language_of_string language)

let%client update_language_handler () language =
  Myshopp_i18n.(set_language (language_of_string language));
