...
let%shared () =
  let registerDemo (module D : Demo_tools.Page) =
    Myshopp_base.App.register ~service:D.service
      ( Myshopp_page.Opt.connected_page @@ fun myid_o () () ->
        let%lwt p = D.page () in
        Myshopp_container.page ~a:[a_class [D.page_class]] myid_o p )
  in
  List.iter registerDemo Demo_tools.demos;
  Myshopp_base.App.register
    ~service:Myshopp_services.demo_service
    (Myshopp_page.Opt.connected_page handler)
...
let%shared () =
  ...
  Myshopp_base.App.register
    ...
    (Myshopp_page.Opt.connected_page detail_page_handler)
